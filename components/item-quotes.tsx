...
export const ItemQuotes = ({ id, is_gold = false, is_xrs = false }: ItemQuotesProps) => {
  if (is_xrs) return null;
  ...
  const columns = [
    ...
    { key: 'size', label: is_gold ? 'Sellers' : 'Orders' },
  ];
          ...
          <TableBody items={data.quotes}>
            {(quote) => (
              <TableRow key={`${quote.price}-${quote.quantity}`}>
                ...
                <TableCell>{quote.open_interest.toLocaleString('ru-RU')}</TableCell>
