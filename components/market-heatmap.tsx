...
export const MarketHeatmap: FC<MarketHeatmapProps> = ({ id, is_commdty = false }) => {
  ...
  const handleMouseMove = (e: React.MouseEvent, point: HeatmapDataPoint) => {
    ...
    <Card className="m-4 p-4 relative">
      <div className="overflow-x-auto">
        <div className="inline-block min-w-full">
          {/* Grid Container */}
          <div className="grid gap-[1px] bg-gray-200" style={{
      ...
      {hoveredCell && (
        ...
        >
          <div className="space-y-1">
            <div>
              <strong>Time:</strong> {formatAxisLabel(data.xAxis[hoveredCell.x])}
            </div>
            <div>
              <strong>Price:</strong> {data.yAxis[hoveredCell.y]}
            </div>
            <div>
              <strong>Quantity:</strong> {hoveredCell.value.toLocaleString('ru-RU')}
            </div>
            ...
            {hoveredCell.oi !== undefined && (
              <div>
                <strong>Open Interest:</strong> {parseInt(String(hoveredCell.oi)).toLocaleString('ru-RU')}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex h-4 w-32">
          {[0, 0.25, 0.5, 0.75, 1].map((intensity, i) => (
            <div
              key={i}
              className="flex-1"
              style={{ backgroundColor: `rgba(167,167,167,${0.1 + intensity * 0.9})` }}
            />
          ))}
        </div>
