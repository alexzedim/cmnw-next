...
import {
  useLiveFeed,
  type LiveFeedStatus,
} from "@/components/providers/live-feed-provider";
...
const FeedRow = ({ event }: { event: FeedEvent }) => {
  const meta =
    FEED_STATUS_META[event.status as FeedStatus] ??
    FEED_STATUS_META[FeedStatus.INFO];
  ...
  return (
    <div className="flex items-start gap-2 px-3 py-1.5 border-b border-border/40 last:border-b-0 hover:bg-foreground/[0.03] transition-colors">
      <span className={`${colorClass} shrink-0 w-4 text-center`}>
        {meta.icon}
      </span>
      <span
        className={`${colorClass} shrink-0 w-16 text-xs uppercase tracking-wide`}
      >
        {meta.statusText}
      </span>
