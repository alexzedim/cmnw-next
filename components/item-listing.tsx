...
import { Card, CardBody, Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, Spinner, Pagination } from "@heroui/react";
...
export const ItemListing = ({ id, name, is_gold = false, is_commdty = false, is_xrs = false }: ItemListingProps) => {
  const [page, setPage] = useState(1);
  const rowsPerPage = 25;

  if (is_commdty || is_gold) return null;
  ...
  const buildWowheadUrl = (auction: Auction) => {
    ...
    wowhead += '&xml';
    return { url: `https://wowhead.com/item=${auction.item_id}`, data: wowhead };
  };
