...
const SEVERITY_STYLES: Record<ToastSeverity, string> = {
  ...
  primary: "bg-[var(--primary)] border-[var(--primary)] text-[var(--primary-foreground)]",
  secondary: "bg-[var(--secondary)] border-[var(--secondary)] text-[var(--secondary-foreground)]",
 ...
 */
export function ToastViewport({ maxVisible = 4 }: { maxVisible?: number }): ReactNode {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {item.description ? (
          <p className="text-sm opacity-90 mt-0.5 leading-snug">{item.description}</p>
        ) : null}
      </div>
      <button
        type="button"
